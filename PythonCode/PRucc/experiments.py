...
def compare_prucc(h=6, n_mpr=5, n_pru=5, fix='mpr', u_l='opt'):
    #h: chosen dataset
    '''
    ...
    u_l:   '#P'  maximum number of permissions per user in UPA
           'opt' use the values in the optimal solution depending on 'fix'
    ...
    for v1 in fixed_list:
        for v2 in to_test[v1]:
            ...
            for test in test_bed:
                ...
                print('PRUCC1: {:>3} {:6d} {:6d}'.format(test[0], nroles, wsc), end=' ')
                print('OK' if istance._check_solution() else 'NOT COVERED')

                istance = PRUCC_2(dataset_name, mpr, mru, test[1], test[2])
                istance.mine()
                wsc, nroles, ua_size, pa_size = istance.get_wsc()
                print('PRUCC2: {:>3} {:6d} {:6d}'.format(test[0], nroles, wsc), end=' ')
                print('OK' if istance._check_solution() else 'NOT COVERED')

                istance = PRUCC_3(dataset_name, mpr, mru, test[1], test[2])
                istance.mine()
                wsc, nroles, ua_size, pa_size = istance.get_wsc()
                print('PRUCC3: {:>3} {:6d} {:6d}'.format(test[0], nroles, wsc), end=' ')
                print('OK' if istance._check_solution() else 'NOT COVERED')
...
def compare_synthetic(nr, nu, np, mru, mpr, runs=5):
    ...
    for test in test_bed:
        ...
        for _ in range(runs):
            ...
            if len(used_roles) != nr:
                print('used ', len(used_roles), 'instead of ', nr)
            ...
            #similarity = compute_sim(istance._pa, pa)
            #similarity = compute_sim(pa, istance._pa)
            #print('pr1', test[0], '#roles:', wsc[1], ' wsc:', wsc[0], ' - found roles: ', found_roles)
            tot_roles_1 += wsc[1]
            tot_wsc_1 += wsc[0]
            tot_found_1 += found_roles / len(used_roles)
            ...
            tot_roles_2 += wsc[1]
            tot_wsc_2 += wsc[0]
            tot_found_2 += found_roles / len(used_roles)
            tot_sim_2 += similarity


        #print('-'*20)
        #we should consider the accuracy with respect the roles used in UPA
        to_return.append((test[0], tot_roles_1, tot_wsc_1, tot_found_1, tot_sim_1, tot_roles_2, tot_wsc_2, tot_found_2, tot_sim_2))
        print('pr1', test[0], '#roles:', tot_roles_1//runs, ' wsc:', tot_wsc_1//runs, ' - accuracy: ', round(tot_found_1/runs, 2), '- similarity: ', round(tot_sim_1/runs, 2))
        print('pr2', test[0], '#roles:', tot_roles_2//runs, ' wsc:', tot_wsc_2//runs, ' - accuracy: ', round(tot_found_2/runs, 2), '- similarity: ', round(tot_sim_2/runs, 2))
